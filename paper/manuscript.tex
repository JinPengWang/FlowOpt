...
\begin{algorithm}[t]
\caption{FlowOpt: Entropic Optimal Transport Flow Matching Optimizer}
\begin{algorithmic}[1]
\REQUIRE Objective $f(x)$, bounds $[lb, ub]$, dimension $D$, budget $T_{\max}$ iterations, population size $N$.
\STATE Initialize $m_0 \sim U[lb, ub]$, $\sigma_0 = 0.3 \cdot \text{span}$, $C_0 = I_D$, $p_\sigma = 0$, $p_c = 0$.
\WHILE{$t < T_{\max}$}
    \STATE Anneal OT entropic noise schedule: $\varepsilon_t = \varepsilon_0 (1 - t/T_{\max}) + \varepsilon_{\min}$.
    ...
    \STATE Compute Gibbs-Boltzmann log-rank weights $w$ over top $\mu$ elites.
    \STATE Solve Entropic Optimal Transport $\Pi^* = \text{Sinkhorn}(X, X, w, \varepsilon_t)$.
    \STATE Compute OT velocities $u^{(i)} = \sum_j \Pi_{ij}^* x^{(j)} - x^{(i)}$.
    \STATE Update mean: $m_{t+1} \leftarrow \text{clamp}\left(m_t + \frac{1}{N}\sum_{i=1}^N u^{(i)}\right)$.
