...
The OT displacement field $u^{(i)} = \hat{y}^{(i)} - x^{(i)}$ integrates to the mean update:
\begin{equation}
m_{t+1} = m_t + \frac{1}{N}\sum_{i=1}^N u^{(i)}
\end{equation}
The CSA path is updated using \textbf{elite weighted residuals}---the direct FM selection signal---with the $\sqrt{c_\sigma(2-c_\sigma)}$ factor guaranteeing stationary $\text{Var}[p_\sigma]=I_D$ (Theorem~3):
\begin{align}
z_w &= \sqrt{\mu_{\text{eff}}} \sum_{i=1}^\mu w_i \frac{x_{(i)} - m_t}{\sigma_t} \\
p_\sigma &\leftarrow (1 - c_\sigma) p_\sigma + \sqrt{c_\sigma(2-c_\sigma)}\, z_w
\end{align}
The covariance matrix uses a \textbf{rank-$\mu$ update only} (no rank-1 path by design):
\begin{equation}
C_{t+1} = (1 - \alpha_C)\,C_t + \alpha_C \sum_{i=1}^{\mu} w_i \frac{(x_{(i)}-m_t)(x_{(i)}-m_t)^T}{\sigma_t^2}
\end{equation}
where $\alpha_C = \mu_\text{eff}/(D^2+\mu_\text{eff})$ is the single rank-$\mu$ rate, replacing the $c_1$/$c_\mu$ split of CMA-ES.
...
\begin{enumerate}\setlength{\itemsep}{1pt}\setlength{\parskip}{0pt}
    \item \textbf{Canonical Entropic Schedule}: The optimal transport entropic parameter $\varepsilon(t) = \frac{1}{2}(1 - t/T_{\max}) + 10^{-3}$ is non-dimensionalized by the median pairwise distance, eliminating temperature tuning.
    \item \textbf{First-Principles Geometric Invariants}: All internal coefficients are derived strictly from $(D, \mu_{\text{eff}})$ without empirical magic numbers: $c_\sigma = \frac{\mu_{\text{eff}}}{D + \mu_{\text{eff}}}$, $d_\sigma = 1 + c_\sigma$, and $\alpha_C = \frac{\mu_{\text{eff}}}{D^2 + \mu_{\text{eff}}}$. The rank-1 CMA-ES parameters ($c_c$, $c_1$) are deliberately omitted: they provide negligible benefit at $N=30$ while adding the most numerically fragile hyperparameter.
...
\STATE Initialize $m_0 \sim \mathcal{U}[lb, ub]$, $\sigma_0 = 0.3 \cdot \text{span}$, $C_0 = I_D$, $p_\sigma = \mathbf{0}$.
\STATE Compute canonical invariants: $c_\sigma = \frac{\mu_{\text{eff}}}{D+\mu_{\text{eff}}}$, $d_\sigma = 1+c_\sigma$, $\alpha_C = \frac{\mu_{\text{eff}}}{D^2+\mu_{\text{eff}}}$.
\WHILE{$t < T_{\max}$}
    \STATE Canonical entropic schedule: $\varepsilon_t \leftarrow \frac{1}{2}(1 - t/T_{\max}) + 10^{-3}$.
    ...
    \STATE Compute log-rank weights $w$ over top $\lfloor N/2 \rfloor$ elites.
    \STATE OT velocity field $u^{(i)} \leftarrow \operatorname{Sinkhorn}(X, w, \varepsilon_t) - x^{(i)}$.
    \STATE Probability flow drift: $m_{t+1} \leftarrow \text{clamp}\left(m_t + \frac{1}{N}\sum_{i=1}^N u^{(i)}\right)$.
    \STATE Elite residuals: $z_w \leftarrow \sqrt{\mu_{\text{eff}}} \sum_{i=1}^\mu w_i \frac{x_{(i)} - m_t}{\sigma_t}$.
    \STATE $p_\sigma \leftarrow (1 - c_\sigma) p_\sigma + \sqrt{c_\sigma (2 - c_\sigma)}\, z_w$.
    ...
    \STATE Rank-$\mu$ covariance: $C_{t+1} \leftarrow (1-\alpha_C)C_t + \alpha_C \sum_{i=1}^\mu w_i \frac{(x_{(i)}-m_t)(x_{(i)}-m_t)^T}{\sigma_t^2}$.
\ENDWHILE
\RETURN $x^*, f^*$
\end{algorithmic}
\end{algorithm}
...
\begin{itemize}\setlength{\itemsep}{1pt}\setlength{\parskip}{0pt}
    \item \textbf{Exact Zero Convex Convergence}: On the Sphere unimodal benchmark, FlowOpt achieves exact numerical convergence of $\mathbf{0.0000 \pm 0.0000}$ on all seeds, outperforming CMA-ES ($1.95 \times 10^{-14}$, $p = 0.0075$ **) as well as PSO ($1.04 \times 10^{-9}$ **), DE ($0.032$ **), CBO ($0.370$ **), and CEM ($0.735$ **). This verifies that calibrated FP-CSA completely eliminates step-size stalling.
    \item \textbf{Ill-Conditioned Valleys}: On the Rosenbrock banana valley, FlowOpt scores $4.948 \pm 0.785$, decisively outperforming DE ($8.614$, $p = 0.0079$ **), CBO ($15.571$, $p = 0.0079$ **), and CEM ($42.984$, $p = 0.0079$ **). Continuous probability transport maintains smooth descent along the parabolic floor without discrete particle pinning.
    \item \textbf{Multimodal Landscapes}: On highly multimodal Rastrigin, FlowOpt attains $\mathbf{3.383 \pm 2.143}$, the lowest mean error among all six evaluated methods, outperforming CMA-ES ($5.572 \pm 1.015$), PSO ($5.974 \pm 2.084$), CEM ($15.291$, $p < 0.01$ **), CBO ($30.017$, $p < 0.01$ **), and DE ($44.434$, $p < 0.01$ **). On Griewank, FlowOpt achieves exact global optimum discovery ($\mathbf{0.0000 \pm 0.0000}$) across 100\% of runs, beating CMA-ES ($1.48 \times 10^{-3}$) and all other baselines ($p < 0.01$ **). On Schwefel, FlowOpt achieves $\mathbf{105.3 \pm 152.5}$, significantly beating CMA-ES ($616.2$, $p = 0.0317$ *), DE ($1207.5$, $p < 0.01$ **), and CBO ($2172.3$, $p < 0.01$ **). On Ackley and Levy, FlowOpt reaches the exact machine resolution limits ($9.54 \times 10^{-7}$ and $7.64 \times 10^{-15}$, beating CMA-ES $p = 0.0073$ **).
    \item \textbf{Molecular Potential Optimization}: On the Lennard-Jones 3-particle atomic cluster, FlowOpt converges to the exact global ground state ($\mathbf{-3.000 \pm 0.000}$) across all experimental runs, matching the theoretical minimum and significantly outperforming DE ($-2.880$, $p = 0.0075$ **) and CBO ($-2.640$, $p = 0.0075$ **).
\end{itemize}
