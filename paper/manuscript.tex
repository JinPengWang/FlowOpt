...
\subsection{Hyperparameter-Free Guarantee and Invariance}
A foundational property of milestone optimization algorithms is the complete elimination of sensitive, problem-dependent hyperparameter tuning. While heuristic algorithms (such as PSO, DE, or CBO) require configuring fragile coefficients (inertia weights, cognitive/social accelerations, mutation/crossover rates, drift friction) that must be re-tuned per landscape, \textbf{FlowOpt is strictly hyperparameter-free}:
\begin{enumerate}\setlength{\itemsep}{1pt}\setlength{\parskip}{0pt}
    ...
    \item \textbf{Analytical Invariants}: All internal coefficients ($c_\sigma, d_\sigma, c_c, c_1, c_\mu$) are closed-form functions of $(D, N)$ derived strictly from the null-hypothesis condition $\mathbb{E}[\|z_{\text{flow}}\|] = \chi_D$ and Riemannian metric positive definiteness.
...
\begin{algorithm}[t]
\caption{FlowOpt: Hyperparameter-Free Riemannian Flow Matching}
\begin{algorithmic}[1]
...
\STATE Compute analytical geometric invariants $(c_\sigma, d_\sigma, c_c, c_1, c_\mu, \chi_D)$ from $(D, N)$.
\WHILE{$t < T_{\max}$}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.68\linewidth]{../figures/fig1_flow_trajectories.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.68\linewidth]{../figures/fig2_convergence_curves.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.60\linewidth]{../figures/fig3_ablation_comparison.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.60\linewidth]{../figures/fig4_neural_vs_closedform.png}
