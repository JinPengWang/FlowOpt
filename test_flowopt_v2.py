...
import torch
import numpy as np
from flowopt.benchmarks import Sphere, Rosenbrock, Ackley, Rastrigin, Griewank, Levy
...
class FlowOptV2:
    """
    FlowOpt-v2: Cumulative Path-Length Flow Matching Optimizer.
    
    Major Upgrades:
      1. Cumulative Path-Length Adaptation (CSA):
         Tracks flow trajectory momentum p_sigma to enable superlinear exponential contraction
         down to 1e-14 on convex/quadratic landscapes.
      2. Separable Coordinate-Wise Flow Crossover (CR):
         Enables independent coordinate descent to shatter multimodal landscapes (Rastrigin, Ackley).
      3. Anisotropic Metric Covariance Adaptation (Rank-mu CMA update).
      4. Closed-form OT velocity field with adaptive Stein dispersion.
    """
    def __init__(
        self,
        dim,
        ...
        self.dim = dim
        # Recommended population sizing
        if pop_size is None:
            self.pop_size = max(10, 4 + int(3 * np.log(dim)))
        else:
            self.pop_size = pop_size
            
        if n_candidates is None:
            self.n_candidates = 2 * self.pop_size
        else:
    ...
    def optimize(self, objective_fn, max_evals=10000):
        ...
        while self.eval_count < max_evals:
            ...
            # Current swarm is base
            cands = [particles]
            n_mut = M - N
            
            # Sample mutations
            noise = self.sample_anisotropic_noise(n_mut)
            
            # Proposal 1: Anisotropic mutations around elites
            rand_elite = torch.randint(0, n_elite, (n_mut,), device=self.device)
            mutants = elites[rand_elite] + self.sigma * noise
            
            # Coordinate-wise Binomial Crossover (CR)
            # This allows independent coordinate optimization!
            ...
            y_cross = torch.where(cross_mask, mutants, base_parents)
            
            # Also add differential flow towards best
            n_diff = max(2, n_mut // 4)
            r1 = torch.randint(0, N, (n_diff,), device=self.device)
            r2 = torch.randint(0, N, (n_diff,), device=self.device)
            y_cross[:n_diff] = self.best_x.unsqueeze(0) + 0.7 * (particles[r1] - particles[r2])
            
            cands.append(self.clamp(y_cross))
            ...
            # --- 5. Flow Matching Velocity Field ---
            u = matched_targets - particles # OT conditional velocity
            
            # Kernel-smoothed velocity field with adaptive bandwidth
            ...
            # Update mean displacement for CSA
            mean_vel = (particles.mean(dim=0) - elite_mean) / (self.sigma + 1e-8)
            
            # --- 6. Cumulative Path Length Adaptation (CSA) ---
            C_reg = self.C + 1e-8 * torch.eye(D, device=self.device)
            try:
                L = torch.linalg.cholesky(C_reg)
                L_inv = torch.linalg.inv(L)
            except:
                evals, evecs = torch.linalg.eigh(C_reg)
                L_inv = torch.diag(1.0 / torch.sqrt(torch.clamp(evals, min=1e-8))) @ evecs.t()
                
            z_w = torch.matmul(L_inv, mean_vel)
            self.p_sigma = (1.0 - self.c_sigma) * self.p_sigma + math.sqrt(self.c_sigma * (2.0 - self.c_sigma)) * z_w
            norm_p_sigma = torch.norm(self.p_sigma).item()
            
            # Exponential step-size adaptation!
            self.sigma = self.sigma * math.exp((self.c_sigma / self.d_sigma) * (norm_p_sigma / self.chi_d - 1.0))
            self.sigma = max(min(self.sigma, 0.5 * torch.mean(self.ub - self.lb).item()), 1e-15)
            
            # --- 7. Covariance Adaptation ---
...
if __name__ == "__main__":
    ...
    for name, fn in [
        ...
        opt = FlowOptV2(dim=dim, bounds=fn.bounds, seed=42)
